(function () {
  ...
  function loadReservas() {
    ...
    return InventarioApp.apiFetch(q).then(function (r) {
      ...
      r.reservas.forEach(function (x) {
        ...
        var acciones = "";
        var esAdmin = currentUser && currentUser.perfil_id === 1;
        var esMia = currentUser && Number(x.usuario_id) === Number(currentUser.id);
        if (esAdmin && x.estado === "pendiente") {
          acciones +=
        ...
        if (esAdmin && (x.estado === "pendiente" || x.estado === "confirmada")) {
          acciones +=
            '<button type="button" class="danger" data-acc="cancelar-admin" data-id="' +
            x.id +
            '">Cancelar</button> ';
        }
        if (!esAdmin && esMia && x.estado === "pendiente") {
          acciones +=
  ...
  InventarioApp.requireAuth().then(function (user) {
    ...
    if (user.perfil_id === 1) linkUsuarios.classList.remove("hidden");
    defaultFiltroFechas();
    loadConfig()
      .then(loadReservas)
      .catch(function (e) {
  ...
  formRes.addEventListener("submit", function (e) {
    ...
    InventarioApp.apiFetch("reservas.php", { method: "POST", body: JSON.stringify(body) })
      .then(function () {
        formRes.reset();
        showOk("Solicitud registrada. Espere confirmación de administración.");
        return loadReservas();
      })
      .catch(function (err) {
        showErr(err.message);
